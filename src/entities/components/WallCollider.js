...
export class WallCollider {
    ...
    update(entity, game) {
        ...
        collided = collided.filter((collision) => {
            ...
            switch (direction) {
                case Direction.North:
                    return (collided.find((c) => equals(tileCoords(c[0].transform.position), [
                        coords[0],
                        coords[1] - 1,
                    ])) === undefined);
        ...
        collided.forEach((collision) => {
            ...
            switch (direction) {
                case Direction.North:
                    entity.transform.position = vec2.fromValues(entity.transform.position[0], value - offset);
                    break;
                case Direction.South:
                    entity.transform.position = vec2.fromValues(entity.transform.position[0], value + offset);
                    break;
                case Direction.East:
                    entity.transform.position = vec2.fromValues(value + offset, entity.transform.position[1]);
                    break;
                case Direction.West:
                    entity.transform.position = vec2.fromValues(value - offset, entity.transform.position[1]);
                    break;
